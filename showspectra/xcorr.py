...
from graphics import drawSpectrum
...
def computeXcorr(wg,fg,eg,template):
    ...
    for i in range(len(nm)):
        ...
        #print " z {0:.5f}".format(z),"{0:.5f}".format(sigmaz)," SNR {0:.3f} ".format(snr[i]), " N ",N
...
def cross_correlation(self):
    ...
    for i in range(len(idxs)):
        ii = idxs[i]
        print "Best xcorr: z {0:.5f}".format(z[ii])," sz {0:.5f}".format(sz[ii]), " snr {0:.3f}".format(snr[ii]), temps[ii]
    ...
    if idx.sum() > 0:
        ...
        drawSpectrum(self)
        # Prompt a dialogue window to show the best 5 x-corr and the possibility to choose a different solution
